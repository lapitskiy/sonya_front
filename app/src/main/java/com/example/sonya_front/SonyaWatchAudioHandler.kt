package com.example.sonya_front

import android.content.Context
import android.content.Intent
import androidx.core.content.ContextCompat
import com.example.sonya_front.AppLog as Log
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.Job
import kotlinx.coroutines.SupervisorJob
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import kotlinx.coroutines.cancel
import java.io.ByteArrayOutputStream
import kotlin.math.abs
import kotlin.math.ceil
import kotlin.math.sqrt

/**
 * Приём и обработка BLE-аудио с часов в рамках foreground-сервиса [VoiceRecognitionService].
 *
 * Живёт независимо от Activity: подписывается на [SonyaWatchBleManager] (который держит один
 * общий [SonyaWatchBleClient] на applicationContext), переваривает фреймы, накапливает PCM,
 * сохраняет WAV, распознаёт речь (Vosk) и отправляет команду через
 * [VoiceRecognitionService.ACTION_PROCESS_RECOGNIZED_TEXT] (source = "watch_vosk").
 *
 * Прогресс/батарея/лог/события пробрасываются наверх через [Callbacks] — сервис транслирует
 * их в broadcasts, чтобы UI вкладки «Часы» обновлялся, когда Activity жива.
 */
class SonyaWatchAudioHandler(
    private val appCtx: Context,
    private val ble: SonyaWatchBleClient,
    private val callbacks: Callbacks,
) : SonyaWatchBleManager.Listener {

    /** Проброс событий, не зависящих от UI, обратно вызывающей стороне (сервису). */
    interface Callbacks {
        /** Контекст для отправки Intent в VoiceRecognitionService. */
        fun serviceContext(): Context

        /** Готовая фраза TTS; сервис озвучит через свой TextToSpeech. */
        fun speak(text: String)

        /** Тактильный отклик при готовности/событии. */
        fun vibrate()

        /** Трансляция статусной строки во вкладку «Часы» (через broadcast). */
        fun broadcastStatus(status: String)

        /** Лог-хвост вкладки «Часы». */
        fun broadcastLog(line: String)

        /** Однострочное событие (lastEvent). */
        fun broadcastEvent(event: String)

        /** Состояние батареи часов. */
        fun broadcastBattery(
            pct: Int?,
            bmv: Int?,
            vbusMv: Int?,
            charging: Boolean?,
            vbusIn: Boolean?,
            batteryPresent: Boolean?,
            etaMin: Int?,
            drainMvPerMin: Int?,
            note: String,
        )

        /** Прогресс передачи аудио. */
        fun broadcastTransfer(totalBytes: Int, offsetBytes: Int, bytesTotal: Long)

        /** Идёт ли запись прямо сейчас. */
        fun broadcastRecordingState(recording: Boolean)

        /** Распознанный текст команды (для UI). */
        fun broadcastTranscript(text: String)

        /** Путь к сохранённому WAV (для ручной загрузки/проигрывания из UI). */
        fun broadcastWavPath(path: String)
    }

    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.IO)

    private val frameParser = SonyaWatchFrameParser()
    private val pcm = ByteArrayOutputStream(256 * 1024)

    private var recording = false
    private var expectedSeq: Int? = null

    private data class RecMeta(
        val recId: Int,
        val totalBytes: Int,
        val crc32: Long,
        val sampleRate: Int,
    )

    private var pendingMeta: RecMeta? = null
    private var pendingOffset: Int = 0
    private var downloading: Boolean = false
    private var pullTimeoutJob: Job? = null
    private var lastGetSentAtMs: Long = 0L
    private val pullWindowBytes: Int = 16 * 1024
    private var pendingWindowEndOffset: Int = 0
    private var lastAudioDataAtMs: Long = 0L
    private var pullStartAtMs: Long = 0L
    private var pullLastReportAtMs: Long = 0L
    private var pullBytesAtLastReport: Int = 0
    private var liveRecId: Int = -1
    private var lastBroadcastProgressPct: Int = -1
    private var pendingDoneRecId: Int = -1
    private var doneRetryJob: Job? = null

    private data class BattPoint(val atMs: Long, val mv: Int)
    private val battHistory = ArrayList<BattPoint>(16)
    private var readyVibrationPending = false

    /** Подписаться на BLE-менеджер и включить авто-подключение. Из onCreate сервиса. */
    fun start() {
        SonyaWatchBleManager.registerListener(appCtx, this)
        ble.setAutoConnectEnabled(true)
    }

    /** Отписаться и выключить авто-подключение. Из onDestroy сервиса. */
    fun stop() {
        SonyaWatchBleManager.unregisterListener(this)
        ble.setAutoConnectEnabled(false)
        scope.cancel()
        doneRetryJob?.cancel()
        pullTimeoutJob?.cancel()
    }

    // ---- SonyaWatchBleManager.Listener ----

    override fun onWatchLog(line: String) {
        callbacks.broadcastLog(line)
    }

    override fun onWatchConnectedChanged(connected: Boolean) {
        callbacks.broadcastLog(if (connected) "BLE connected" else "BLE disconnected")
        if (connected) {
            readyVibrationPending = true
            callbacks.broadcastEvent("BLE подключено (готовлю GATT)…")
        } else {
            readyVibrationPending = false
            resetProtocolState()
            callbacks.broadcastEvent("Ожидаю подключения к часам…")
        }
        callbacks.broadcastRecordingState(false)
    }

    override fun onWatchScanningChanged(scanning: Boolean) {
        // ViewModel получает scanning через собственную подписку на BleManager.
    }

    override fun onWatchGattReady() {
        callbacks.broadcastEvent("BLE готово (жду PONG)…")
        scope.launch {
            sendPing()
            delay(1500L)
            sendPing()
        }
    }

    override fun onWatchNotifyBytes(bytes: ByteArray) {
        val frames = frameParser.push(bytes)
        if (frames.isEmpty()) return
        frames.forEach { handleFrame(it) }
    }

    // ---- Frame handling (переехало из SonyaWatchViewModel) ----

    private fun resetProtocolState() {
        recording = false
        downloading = false
        expectedSeq = null
        pendingMeta = null
        pendingOffset = 0
        liveRecId = -1
        lastBroadcastProgressPct = -1
        pendingDoneRecId = -1
        doneRetryJob?.cancel()
        doneRetryJob = null
        pullTimeoutJob?.cancel()
        pullTimeoutJob = null
        callbacks.broadcastTransfer(totalBytes = 0, offsetBytes = 0, bytesTotal = 0)
    }

    private fun handleFrame(f: SonyaWatchFrame) {
        val typeName = when (f.type) {
            SonyaWatchProtocol.EVT_WAKE -> "EVT_WAKE"
            SonyaWatchProtocol.EVT_REC_START -> "EVT_REC_START"
            SonyaWatchProtocol.EVT_REC_END -> "EVT_REC_END"
            SonyaWatchProtocol.AUDIO_CHUNK -> "AUDIO_CHUNK"
            SonyaWatchProtocol.AUDIO_DATA -> "AUDIO_DATA"
            SonyaWatchProtocol.EVT_ERROR -> "EVT_ERROR"
            else -> "0x" + f.type.toString(16)
        }

        if (expectedSeq != null && f.type != SonyaWatchProtocol.AUDIO_DATA) {
            val exp = expectedSeq ?: 0
            if (f.seq != exp) {
                callbacks.broadcastLog("seq mismatch: got=${f.seq} expected=$exp type=$typeName")
            }
            expectedSeq = (f.seq + 1) and 0xFFFF
        }

        when (f.type) {
            SonyaWatchProtocol.EVT_WAKE -> {
                callbacks.broadcastLog(
                    "wake: rec=$recording dl=$downloading pendingMeta=${pendingMeta?.recId ?: -1} off=$pendingOffset"
                )
                callbacks.broadcastEvent("$typeName seq=${f.seq}")
            }

            SonyaWatchProtocol.EVT_REC_START -> {
                callbacks.broadcastLog(
                    "rec_start: before rec=$recording dl=$downloading pendingMeta=${pendingMeta?.recId ?: -1} off=$pendingOffset liveRecId=$liveRecId"
                )
                recording = true
                downloading = false
                expectedSeq = (f.seq + 1) and 0xFFFF
                pcm.reset()
                pendingMeta = null
                pendingOffset = 0
                liveRecId = -1
                pullTimeoutJob?.cancel()
                pullTimeoutJob = null
                pullStartAtMs = System.currentTimeMillis()
                pullLastReportAtMs = 0L
                pullBytesAtLastReport = 0
                lastBroadcastProgressPct = -1
                callbacks.broadcastTransfer(totalBytes = 0, offsetBytes = 0, bytesTotal = 0)
                callbacks.broadcastEvent("$typeName seq=${f.seq}")
                callbacks.broadcastRecordingState(true)
                callbacks.broadcastStatus("Часы: запись началась")
            }

            SonyaWatchProtocol.AUDIO_CHUNK -> {
                if (!recording) {
                    callbacks.broadcastLog("AUDIO_CHUNK while not recording: ${f.payload.size} bytes")
                    return
                }
                pcm.write(f.payload)
                val total = pcm.size().toLong()
                callbacks.broadcastTransfer(totalBytes = 0, offsetBytes = 0, bytesTotal = total)
                if (total % (64 * 1024) < f.payload.size) {
                    callbacks.broadcastLog("audio bytes total=$total")
                }
            }

            SonyaWatchProtocol.AUDIO_DATA -> {
                if (f.payload.size < 6) {
                    callbacks.broadcastLog("AUDIO_DATA too short: ${f.payload.size}")
                    return
                }
                val recId = u16le(f.payload, 0)
                val off = u32le(f.payload, 2)
                val data = f.payload.copyOfRange(6, f.payload.size)

                if (recording && pendingMeta == null) {
                    // Live mode: data arriving while still recording
                    if (liveRecId == -1) {
                        liveRecId = recId
                        callbacks.broadcastLog("live: first frame recId=$recId len=${data.size}")
                    }
                    if (recId != liveRecId) return
                    if (off != pendingOffset) {
                        callbacks.broadcastLog("live: offset gap expected=$pendingOffset got=$off, padding ${off - pendingOffset}B")
                        val gap = off - pendingOffset
                        if (gap in 1..4096) {
                            pcm.write(ByteArray(gap))
                            pendingOffset += gap
                        } else {
                            return
                        }
                    }
                    pcm.write(data)
                    pendingOffset += data.size
                    callbacks.broadcastTransfer(totalBytes = 0, offsetBytes = 0, bytesTotal = pcm.size().toLong())
                    reportThroughput(data.size)
                    return
                }

                // Pull mode: downloading after REC_END
                val m = pendingMeta
                if (m == null) {
                    callbacks.broadcastLog("AUDIO_DATA but no context: ${f.payload.size} bytes")
                    return
                }
                pullTimeoutJob?.cancel()
                pullTimeoutJob = null
                lastAudioDataAtMs = System.currentTimeMillis()
                if (recId != m.recId) return
                if (off != pendingOffset) {
                    callbacks.broadcastLog("pull: offset mismatch got=$off expected=$pendingOffset")
                    requestWindow(fromOffset = pendingOffset)
                    return
                }
                pcm.write(data)
                pendingOffset += data.size
                callbacks.broadcastTransfer(
                    totalBytes = m.totalBytes,
                    offsetBytes = pendingOffset,
                    bytesTotal = pcm.size().toInt(),
                )
                reportThroughput(data.size)
                maybeBroadcastTransferProgress(total = m.totalBytes, offset = pendingOffset)

                if (pendingOffset < m.totalBytes) {
                    if (pendingOffset >= pendingWindowEndOffset) {
                        requestWindow(fromOffset = pendingOffset)
                    } else {
                        schedulePullTimeout()
                    }
                } else {
                    finalizeDone(m)
                }
            }

            SonyaWatchProtocol.EVT_REC_END -> {
                callbacks.broadcastEvent("$typeName seq=${f.seq}")
                callbacks.broadcastStatus("Часы: запись завершена, принимаю данные")
                val meta = parseRecEndMeta(f.payload)
                if (meta == null) {
                    recording = false
                    downloading = false
                    expectedSeq = null
                    val pcmBytes = pcm.toByteArray()
                    callbacks.broadcastLog("rec end (legacy): pcmBytes=${pcmBytes.size}")
                    saveWav(pcmBytes)
                } else {
                    recording = false
                    callbacks.broadcastLog("rec meta: recId=${meta.recId} totalBytes=${meta.totalBytes} crc32=0x${meta.crc32.toString(16)} sr=${meta.sampleRate} liveGot=$pendingOffset")

                    if (pendingOffset >= meta.totalBytes) {
                        callbacks.broadcastLog("live: all data received, finalizing immediately")
                        finalizeDone(meta)
                    } else {
                        pendingMeta = meta
                        pendingWindowEndOffset = 0
                        downloading = true
                        lastAudioDataAtMs = 0L
                        callbacks.broadcastTransfer(
                            totalBytes = meta.totalBytes,
                            offsetBytes = pendingOffset,
                            bytesTotal = pcm.size().toInt(),
                        )
                        val remaining = meta.totalBytes - pendingOffset
                        callbacks.broadcastLog("live: missing ${remaining}B from off=$pendingOffset, pulling remainder")
                        requestWindow(fromOffset = pendingOffset)
                    }
                }
            }

            SonyaWatchProtocol.EVT_ERROR -> {
                val msg = try {
                    f.payload.toString(Charsets.US_ASCII)
                } catch (_: Throwable) {
                    "<decode error>"
                }
                val m = msg.trim()
                val isBatt = m.startsWith("BATT:")
                if (isBatt) {
                    handleBatteryInfo(m)
                    return
                }
                if (m.startsWith("DONE_OK:")) {
                    val id = m.removePrefix("DONE_OK:").toIntOrNull()
                    if (id != null && id == pendingDoneRecId) {
                        callbacks.broadcastLog("done ack: recId=$id")
                        pendingDoneRecId = -1
                        doneRetryJob?.cancel()
                        doneRetryJob = null
                    }
                    return
                }
                if (m == "REC_BUSY:WAIT_DONE") {
                    callbacks.broadcastLog("watch busy: waiting DONE, retrying last DONE if needed")
                    if (pendingDoneRecId > 0) {
                        ble.writeAsciiCommand("DONE:$pendingDoneRecId")
                    }
                    callbacks.broadcastEvent("WATCH: $m")
                    return
                }
                val isInfo = m == "PONG" || m == "TIME_OK" || m == "TIME_REQ" || m.startsWith("REC_SEC=")
                if (isInfo) {
                    callbacks.broadcastLog("watch: '$m'")
                    callbacks.broadcastEvent("WATCH: $m")
                    if (m == "PONG" || m == "TIME_REQ") {
                        sendTimeSync()
                    }
                    if (m == "PONG" && readyVibrationPending) {
                        readyVibrationPending = false
                        callbacks.vibrate()
                        callbacks.speak(VoiceResponsesConfig.pickWakeResponse(appCtx))
                    }
                } else {
                    callbacks.broadcastLog("watch error: '$m'")
                    callbacks.broadcastEvent("$typeName: $m")
                }
            }

            else -> {
                callbacks.broadcastLog("frame $typeName seq=${f.seq} len=${f.payload.size}")
            }
        }
    }

    private fun handleBatteryInfo(msg: String) {
        if (msg.startsWith("BATT:err=")) {
            callbacks.broadcastLog("watch battery error: $msg")
            callbacks.broadcastEvent("WATCH: $msg")
            return
        }
        val body = msg.removePrefix("BATT:")
        val pairs = body.split(",")
            .mapNotNull { part ->
                val idx = part.indexOf('=')
                if (idx <= 0 || idx >= part.length - 1) null else {
                    part.substring(0, idx).trim() to part.substring(idx + 1).trim()
                }
            }
            .toMap()
        val pct = pairs["pct"]?.toIntOrNull()
        val bmv = pairs["bmv"]?.toIntOrNull()
        val vbus = pairs["vbus"]?.toIntOrNull()
        val chg = when (pairs["chg"]) {
            "1" -> true
            "0" -> false
            else -> null
        }
        val vin = when (pairs["in"]) {
            "1" -> true
            "0" -> false
            else -> null
        }
        val bat = when (pairs["bat"]) {
            "1" -> true
            "0" -> false
            else -> null
        }
        val nowMs = System.currentTimeMillis()
        val chargingNow = (vin == true) || (chg == true) || ((vbus ?: 0) >= 3900)
        val etaMin: Int?
        val drainRateInt: Int?
        if (chargingNow || bmv == null) {
            etaMin = null
            drainRateInt = null
            battHistory.clear()
        } else {
            battHistory.add(BattPoint(nowMs, bmv))
            val minTs = nowMs - 8 * 60_000L
            while (battHistory.isNotEmpty() && battHistory.first().atMs < minTs) {
                battHistory.removeAt(0)
            }
            while (battHistory.size > 16) {
                battHistory.removeAt(0)
            }

            val rate = estimateDrainRateMvPerMin(battHistory)
            drainRateInt = rate?.toInt()
            val vMin = 3500
            etaMin = if (rate != null && rate >= 1.0) {
                val remainingMv = (bmv - vMin).coerceAtLeast(0)
                if (remainingMv == 0) 0 else ceil(remainingMv.toDouble() / rate).toInt().coerceIn(1, 24 * 60)
            } else {
                null
            }
        }
        val note = when {
            bat == false -> "АКБ не обнаружен"
            chargingNow -> "Заряжается"
            pct == 0 && (bmv ?: 0) >= 3600 -> "Процент PMU недостоверен, смотрите mV"
            else -> ""
        }
        callbacks.broadcastBattery(
            pct = pct,
            bmv = bmv,
            vbusMv = vbus,
            charging = chg,
            vbusIn = vin,
            batteryPresent = bat,
            etaMin = etaMin,
            drainMvPerMin = drainRateInt,
            note = note,
        )
        callbacks.broadcastLog("watch battery: pct=$pct bmv=$bmv vbus=$vbus chg=$chg in=$vin bat=$bat")
        callbacks.broadcastEvent("WATCH: battery ${pct ?: "?"}%")
    }

    private fun finalizeDone(m: RecMeta) {
        callbacks.broadcastLog("finalize: recId=${m.recId} expected=${m.totalBytes} got=${pcm.size()} off=$pendingOffset")
        sendDoneWithRetry(m.recId)
        recording = false
        downloading = false
        expectedSeq = null
        liveRecId = -1
        pullTimeoutJob?.cancel()
        pullTimeoutJob = null
        val pcmBytes = pcm.toByteArray()
        callbacks.broadcastLog("rec done: pcmBytes=${pcmBytes.size} expected=${m.totalBytes}")
        callbacks.broadcastStatus("Часы: данные получены, распознаю речь")
        saveWav(pcmBytes)
    }

    private fun sendDoneWithRetry(recId: Int) {
        pendingDoneRecId = recId
        doneRetryJob?.cancel()
        ble.writeAsciiCommand("DONE:$recId")
        doneRetryJob = scope.launch {
            repeat(5) { idx ->
                delay(700L)
                if (pendingDoneRecId != recId) return@launch
                callbacks.broadcastLog("done retry #${idx + 1}: recId=$recId")
                ble.writeAsciiCommand("DONE:$recId")
            }
            if (pendingDoneRecId == recId) {
                callbacks.broadcastLog("done ack timeout: recId=$recId")
            }
        }
    }

    private fun reportThroughput(chunkSize: Int) {
        val now = System.currentTimeMillis()
        if (pullStartAtMs == 0L) pullStartAtMs = now
        if (pullLastReportAtMs == 0L) {
            pullLastReportAtMs = now
            pullBytesAtLastReport = pendingOffset
        }
        val dt = now - pullLastReportAtMs
        if (dt >= 1000L) {
            val dBytes = pendingOffset - pullBytesAtLastReport
            val bps = if (dt > 0) (dBytes.toDouble() * 1000.0 / dt.toDouble()) else 0.0
            val kbps = bps / 1024.0
            val totalDt = now - pullStartAtMs
            callbacks.broadcastLog("stream: off=$pendingOffset chunk=${chunkSize}B rate=${"%.1f".format(kbps)}KiB/s elapsed=${(totalDt / 1000.0).toInt()}s")
            pullLastReportAtMs = now
            pullBytesAtLastReport = pendingOffset
        }
    }

    private fun requestWindow(fromOffset: Int) {
        val m = pendingMeta ?: return
        val remaining = m.totalBytes - fromOffset
        if (remaining <= 0) return
        val want = minOf(pullWindowBytes, remaining)
        pendingWindowEndOffset = fromOffset + want
        lastGetSentAtMs = System.currentTimeMillis()
        callbacks.broadcastLog("GET -> recId=${m.recId} off=$fromOffset len=$want winEnd=$pendingWindowEndOffset total=${m.totalBytes}")
        ble.writeAsciiCommand("GET:${m.recId}:$fromOffset:$want")
        schedulePullTimeout()
    }

    private fun schedulePullTimeout() {
        pullTimeoutJob?.cancel()
        pullTimeoutJob = scope.launch {
            delay(1200L)
            val m2 = pendingMeta ?: return@launch
            if (!downloading) return@launch
            if (pendingOffset >= m2.totalBytes) return@launch
            val now = System.currentTimeMillis()
            val lastData = lastAudioDataAtMs
            val stalled = if (lastData != 0L) (now - lastData) >= 1100L else (now - lastGetSentAtMs) >= 1100L
            if (!stalled) return@launch
            val winEnd = pendingWindowEndOffset
            if (pendingOffset < winEnd) {
                val retryLen = (winEnd - pendingOffset).coerceAtLeast(1)
                callbacks.broadcastLog("pull: stall -> retry window off=$pendingOffset len=$retryLen")
                requestWindow(fromOffset = pendingOffset)
            }
        }
    }

    private fun maybeBroadcastTransferProgress(total: Int, offset: Int) {
        if (total <= 0) return
        val pct = ((offset * 100L) / total.toLong()).toInt().coerceIn(0, 100)
        val shouldSend = when {
            pct >= 100 && lastBroadcastProgressPct < 100 -> true
            lastBroadcastProgressPct < 0 -> true
            pct - lastBroadcastProgressPct >= 10 -> true
            else -> false
        }
        if (!shouldSend) return
        lastBroadcastProgressPct = pct
        callbacks.broadcastStatus("Часы: передача аудио $pct%")
    }

    private fun parseRecEndMeta(payload: ByteArray): RecMeta? {
        if (payload.size != 12) return null
        val recId = u16le(payload, 0)
        val total = u32le(payload, 2)
        val crc = u32le(payload, 6).toLong() and 0xFFFF_FFFFL
        val sr = u16le(payload, 10)
        if (total <= 0) {
            callbacks.broadcastLog("rec meta invalid: recId=$recId total=$total crc=0x${crc.toString(16)} sr=$sr")
            return null
        }
        if (total > 10_000_000) {
            callbacks.broadcastLog("rec meta too large: recId=$recId total=$total")
            return null
        }
        return RecMeta(recId = recId, totalBytes = total, crc32 = crc, sampleRate = sr)
    }

    private fun u16le(b: ByteArray, off: Int): Int {
        val b0 = b[off].toInt() and 0xFF
        val b1 = b[off + 1].toInt() and 0xFF
        return b0 or (b1 shl 8)
    }

    private fun u32le(b: ByteArray, off: Int): Int {
        val b0 = b[off].toInt() and 0xFF
        val b1 = b[off + 1].toInt() and 0xFF
        val b2 = b[off + 2].toInt() and 0xFF
        val b3 = b[off + 3].toInt() and 0xFF
        return b0 or (b1 shl 8) or (b2 shl 16) or (b3 shl 24)
    }

    private fun saveWav(pcmBytes: ByteArray) {
        scope.launch {
            try {
                val dir = appCtx.cacheDir
                val f = SonyaWatchWav.writePcm16leMono16kHzWav(dir = dir, pcmS16Le = pcmBytes)
                callbacks.broadcastLog("WAV saved: ${f.absolutePath} (${f.length()} bytes)")
                callbacks.broadcastWavPath(f.absolutePath)

                val stats = pcmLevelStats(pcmBytes)
                callbacks.broadcastLog("pcm level: samples=${stats.samples} maxAbs=${stats.maxAbs} rms=${"%.4f".format(stats.rms)}")
                if (stats.maxAbs < 80) {
                    callbacks.broadcastLog("pcm looks like silence (maxAbs<80)")
                } else if (stats.maxAbs < 2000) {
                    callbacks.broadcastLog("pcm very quiet (maxAbs<2000), recognition can be blank")
                }

                val text = runCatching {
                    SonyaWatchVoskTranscriber.transcribePcm16leMono16k(appCtx, pcmBytes)
                }.getOrElse { t ->
                    callbacks.broadcastLog("transcribe failed: ${t.javaClass.simpleName}: ${t.message}")
                    ""
                }.trim()

                if (text.isNotBlank()) {
                    callbacks.broadcastLog("watch transcript: '$text'")
                    callbacks.broadcastTranscript(text)
                    try {
                        val ctx = callbacks.serviceContext()
                        val intent = Intent(ctx, VoiceRecognitionService::class.java).apply {
                            action = VoiceRecognitionService.ACTION_PROCESS_RECOGNIZED_TEXT
                            putExtra(VoiceRecognitionService.EXTRA_RECOGNIZED_TEXT, text)
                            putExtra(VoiceRecognitionService.EXTRA_RECOGNIZED_SOURCE, "watch_vosk")
                        }
                        callbacks.broadcastStatus("Часы: отправляю команду на сервер")
                        ContextCompat.startForegroundService(ctx, intent)
                    } catch (t: Throwable) {
                        callbacks.broadcastLog("backend command failed: ${t.javaClass.simpleName}: ${t.message}")
                        callbacks.broadcastStatus("Часы: ошибка отправки команды")
                    }
                } else {
                    callbacks.broadcastLog("watch transcript: <blank>")
                    callbacks.broadcastStatus("Часы: речь не распознана, команда не отправлена")
                    callbacks.broadcastTranscript("")
                }
            } catch (t: Throwable) {
                Log.w(SonyaWatchProtocol.TAG, "saveWav failed: ${t.javaClass.simpleName}: ${t.message}", t)
                callbacks.broadcastLog("saveWav failed: ${t.javaClass.simpleName}: ${t.message}")
            }
        }
    }

    private data class PcmStats(val samples: Int, val maxAbs: Int, val rms: Double)

    private fun pcmLevelStats(pcmBytes: ByteArray): PcmStats {
        val n = pcmBytes.size / 2
        if (n <= 0) return PcmStats(samples = 0, maxAbs = 0, rms = 0.0)
        var maxAbs = 0
        var sumSq = 0.0
        var i = 0
        while (i + 1 < pcmBytes.size) {
            val lo = pcmBytes[i].toInt() and 0xFF
            val hi = pcmBytes[i + 1].toInt()
            val s = (hi shl 8) or lo
            val v = s.toShort().toInt()
            val a = abs(v)
            if (a > maxAbs) maxAbs = a
            sumSq += (v.toDouble() * v.toDouble())
            i += 2
        }
        val rms = sqrt(sumSq / n.toDouble()) / 32768.0
        return PcmStats(samples = n, maxAbs = maxAbs, rms = rms)
    }

    private fun sendPing() {
        ble.writeAsciiCommand("PING")
        sendTimeSync()
    }

    private fun sendTimeSync() {
        val nowMs = System.currentTimeMillis()
        val epochSec = nowMs / 1000L
        val tzOffsetMin = java.util.TimeZone.getDefault().getOffset(nowMs) / 60_000
        ble.writeAsciiCommand("TIME:$epochSec:$tzOffsetMin")
    }

    private fun estimateDrainRateMvPerMin(points: List<BattPoint>): Double? {
        if (points.size < 4) return null
        val t0 = points.first().atMs
        val xs = points.map { (it.atMs - t0).toDouble() / 60_000.0 } // minutes
        val ys = points.map { it.mv.toDouble() }

        val n = xs.size.toDouble()
        val sumX = xs.sum()
        val sumY = ys.sum()
        val sumXY = xs.zip(ys).sumOf { (x, y) -> x * y }
        val sumX2 = xs.sumOf { it * it }
        val denom = n * sumX2 - sumX * sumX
        if (denom <= 1e-9) return null

        val slopeMvPerMin = (n * sumXY - sumX * sumY) / denom
        val drain = (-slopeMvPerMin).coerceAtLeast(0.0)
        if (drain > 300.0) return null
        return drain
    }
}
